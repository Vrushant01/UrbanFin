...
import { type CustomerInvoice, type Contact, type Payment, PaymentType, PaymentVia } from '../../types';
...
export function InvoicePaymentModal({ invoice, customer, amountDue, onClose, onSuccess }: InvoicePaymentModalProps) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center gap-2 text-indigo-700">
            <Receipt size={20} />
            <h3 className="font-bold text-lg">Register Payment (Receive)</h3>
          </div>
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <p className="font-semibold text-slate-800">{customer?.name || 'Unknown'}</p>
            </div>
          </div>
